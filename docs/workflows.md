...
9. `python build_review_package.py --mode discovery --inputs-dir ../batches/staging/<scope-slug> --output ../batches/lng_terminals_batch_<YYYYMMDD>_<HHMM>_ET_<scope>_discovery.xlsx` → staging xlsx (Eastern timestamp via `TZ=America/New_York date "+%Y%m%d_%H%M_ET"`; scope slug per the naming convention above).
...
4. `python scripts/build_review_package.py --mode update --inputs-dir batches/staging/<region>/_build --gem-csv scripts/gem_export.csv --output batches/lng_terminals_batch_<stamp>_<region>_update.xlsx` (plus a `--mode discovery` build → `…_<region>_discovery.xlsx` if the region produced monitor/new candidates), then `python scripts/recalc.py <xlsx>`.
