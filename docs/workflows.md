...
6. Route findings per Reconciliation SOP §4:
   ...
   - GEM-only → log in `routing` sheet, usually no action (GIIGNL has known gaps per the methodology FAQ)
...
7a. **Execute the research pass** (this is what makes the batch actionable rather than a punt — Reconciliation SOP §3.8.5). For every matched-with-disagreement finding that is a *material* conflict (≥5% capacity delta, or a non-benign owner change — not a JV-vs-immediate-owner naming variant or a metric-basis capacity difference), actually research it now instead of deferring to a separate Update batch:
   ...
   - Run `python scripts/entity_lookup.py "<entity>"` before staging any new owner/operator name (don't create duplicate entities).
...
8. `python scripts/build_review_package.py --mode reconciliation --report giignl --year <YEAR> --inputs-dir batches/staging/recon/giignl<YEAR> --gem-csv scripts/gem_export.csv --extracted-csv batches/staging/recon/giignl<YEAR>/giignl_extracted.csv --output batches/lng_terminals_batch_<YYYYMMDD>_<HHMM>_ET_giignl<YEAR>_reconciliation.xlsx` → staging xlsx with the four reconciliation sheets — `audit_operating` / `audit_nonoperating` (the evidence layer), `routing` (non-edit findings: Discovery / gem_only / narrative), `edits_to_gem` (the PASTE VIEW — only rows the research pass resolved to a DB change, with resolved values in the real GEM columns + a leftmost `_change` explanation) — plus `giignl_fsru_fleet` and the standard sheets. (`country_notes_contributions` is NOT emitted in reconciliation mode.) Stamp via `TZ=America/New_York date "+%Y%m%d_%H%M_ET"`.
