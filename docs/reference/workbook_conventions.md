...
| `routing` | Reconciliation workflow | NON-edit findings categorized for Discovery / Review (GIIGNL-only, gem_only, ambiguous, narrative). Matched-with-disagreement value conflicts are NOT here — they go to `edits_to_gem` once researched. |
...
## Color conventions (cells in `updates`, `new_units`, `audit_operating`, `edits_to_gem`)
