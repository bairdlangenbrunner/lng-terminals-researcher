...
**A planned-startup slip IS a status-timeline change — same rule, same elevation.** Read every source you cite (for any field) in full for schedule content, not just the fact you came for. If a source states a revised planned startup/in-service date that differs from GEM's `LatestPlannedStartYear` — or contradicts the newest `operating/planned` timeline entry — that is a staged edit, not prose: (a) `LatestPlannedStartYear` update + `StartDate [ref]`, (b) an `operating`/`planned` timeline append at the new year (pull the timeline first as usual), (c) `OriginalPlannedStartYear` untouched (it is correctly historical). Corroborate the new date like any other value (≥2 independent sources; a slip reported by one outlet citing a named original reporter, e.g. MEES, plus a second independent publisher counts). A source's conflicting official line ("the sponsor says still on track") does not veto a well-corroborated slip — record the slip and note the sponsor's claim in `source_notes`; deferring the year edit to a qa/source note because the sponsor disagrees is the same hedge-and-punt failure as the status case. (This rule exists because of the NFE miss, gulf-turkiye 2026-07: the constructionreviewonline article was cited for construction-resumption while its "first train pushed to early 2027" line — a LatestPlannedStartYear 2026→2027 edit — was left buried in a qa note as "not clearly wrong". A relative schedule anchor counts too: the same article's "NFS ~12–18 months after the first NFE train" line is what re-confirms NFS's 2028.)
...
The gate covers **every URL in every lane** — updates `ref_urls`/`[ref]` new_values, wiki `source_urls`, monitor `best_lead_url`, qa citations — not just the updates sheet. And a citation must be the **specific page** that contains the claimed value: a bare domain/homepage (`https://outlet.com/` — scheme + host, no path) is never a citation, can never legitimately pass the token check, and is flagged by `build_review_package.py`'s bare-domain guard (`warn_bare_domain_urls`, all lanes, both modes). If you read an article, cite the article URL, not the outlet (gulf-turkiye Dörtyol wiki-row miss, 2026-07-17).
...
**Bot-block ≠ dead.** A 401/403/429 (or a 200 Cloudflare/paywall interstitial) means the page is live but refusing bots, NOT gone. `url_verifier.py` automatically falls back to the newest Wayback Machine snapshot on these and runs the same content check against it — a pass means the LIVE URL is verified and stays citable (never cite the web.archive.org address itself). Only a hard 404/410/DNS failure, or a live page that no longer contains the cited value, is a genuinely failed citation. A 301/302 to a live page is a host/path move, not rot — the citation survives as the redirect target (e.g. hydrocarbons-technology.com → offshore-technology.com); record the final URL.

Existing URLs that genuinely fail re-verification (dead, or content gone — not merely bot-blocked) go in `qa_review` with proposed action: either find a replacement URL or blank the `[ref]` cell (and the paired data if Rule F requires it).

**Exhaustive tier: run a mechanical dead-domain sweep, don't rely on per-row diligence.** Some rot is systematic, not per-URL — legacy `giignl.org/...` citations are dead site-wide (discovered Türkiye/Kuwait 2026-07-14; the gulf-turkiye Qatar/UAE cells were then missed because only some country subagents swept for it). At the start of an exhaustive batch, grep the fresh export's in-scope rows for known-dead domains (`giignl.org` at minimum) and put every hit on the worklist mechanically. GIIGNL repairs re-cite the **same edition** at its official mirror (giignl.org/annual-report → elfsightcdn / cdn.prod.website-files.com; exact per-edition URLs in `data/README.md`) after confirming the value is in that edition; if the value isn't in the cited edition, that's a miscitation — find the real provenance instead of rehosting.

### §7.2a Ref edits use MERGE semantics — never drop a surviving URL

When an update record targets a `[ref]` column, its `new_value` **replaces the whole cell**, so it must carry forward **every still-valid existing URL** in addition to any new citations: `new_value` = (surviving existing URLs, in their original order) + (new URLs). The point of a ref edit is only ever to (a) fix genuinely dead/wrong URLs or (b) supplement with more corroboration — never to swap a good existing citation for the agent's own find. Dropping a live existing ref was the Al Zour / gulf-turkiye miss (2026-07-17): a bot-blocked 403 was misread as dead and a correct, live existing citation was overwritten.

**Merge semantics apply to BOTH record shapes.** A `<field> [ref]` record carries the existing cell in its `old_value`, so the merge is visible in the record itself. A **value** record that carries its citations in `ref_urls` (the captive-power / §9 shape: `field_name = CaptiveGasPower`, `ref_urls = [...]`) writes the same ref cell without ever showing its old contents — so its `ref_urls` must ALSO carry forward every surviving URL already in `<field> [ref]`. Check the fresh export's ref cell before staging; don't assume a value record can't drop anything. This shape was invisible to the `REF-DROP:` guard until 2026-07-27, which is how three undeclared drops (Alaska LNG, CP2, Delfin) reached built captive-power workbooks; the guard now reads the target ref cell from the fresh CSV for value records too.

Rules:
- An existing URL may be dropped ONLY if it is proven dead per §7.2 (hard 404/410/DNS, or live-but-content-gone; a bot-block that fails even the Wayback check goes to `qa_review`, not a silent drop) — **or if it is a banned/circular source** (abarrelfull, gem.wiki/globalenergymonitor.org), which must be removed whatever its link status.
- **A banned-source removal is declared in `dropped_urls_banned`, NOT `dropped_urls_dead`.** The banned page is usually still live, so calling it dead would be a false declaration (same principle as the Mugardos case below). The `REF-DROP:` guard accepts either key. `audit_ref_drops.py` classifies these `dropped_banned_domain` and — critically — never restores them even when the probe finds them live, because restoring one would reintroduce a forbidden citation. Removing a banned URL still obeys merge semantics: keep every other existing URL, and if the banned URL was the ONLY support for the value, find a replacement or leave the value with a `qa_review` note rather than orphaning it.
- A deliberate drop is **declared** by listing the dead URL(s) in the record's `dropped_urls_dead` key. `build_review_package.py` prints a `REF-DROP:` guard warning for any `[ref]` record whose `new_value` drops an `old_value` URL without that declaration. The guard treats an old URL whose exact path survives in `new_value` at a different host as KEPT (the 301-rehost case, e.g. hydrocarbons-technology → offshore-technology) — no declaration needed; a redirect the guard can't see offline (e.g. a bit.ly shortlink to a kept page) still needs a `dropped_urls_dead` entry with a `source_notes` disposition.
- Same-document re-hosting (a dead giignl.org PDF replaced by the same edition at GIIGNL's CDN; a domain migration with the same path) counts as keeping the URL, not dropping it — cite the live host.
- **A live page that doesn't contain the TERMINAL NAME is not a drop candidate.** "Live-but-content-gone" above means the page no longer contains the **cited value**; name-absence is advisory only (QC SOP §3.2 — GEM names are routinely suffixed or translated relative to how sources write them), and an owner's/operator's own corporate page legitimately never names the terminal while being the primary source for the very fact cited. Declaring such a URL in `dropped_urls_dead` is also a false declaration. Caught 2026-07-29 on Mugardos `Owner [ref]`, where a sweep agent dropped Reganosa's own live shareholder-disclosure page for lacking the token "Mugardos" — reversed, and the record went red-on-one-URL back to yellow-on-two. When re-verifying a ref, test the **claimed value**, not the terminal name.
- Audit/repair tool: `scripts/audit_ref_drops.py <staging-dir>` classifies every dropped URL (report), and `--apply` merges live ones back and stamps `dropped_urls_dead`. Since 2026-07-27 it audits BOTH record shapes like the guard — value records' target ref cell is read from the fresh export (`--gem-csv`, default `scripts/gem_export.csv`) — and skips URLs already declared in `dropped_urls_dead` or `dropped_urls_banned`.
...
1. `python entity_lookup.py "<entity name>" --pg` — searches the local export AND `entity_history` in the read-only Postgres. **`--pg` is the authoritative check**; `--remote` (the web endpoint) is a supplement that needs `GEM_PROJECT_DB_BASE_URL` and returns `skipped_no_base_url` when it is unset — an environmental SKIP, never a not-found, and never grounds to stage a new entity. **Entities get renamed**, so also try the abbreviation and any former name before concluding absence (the 2026-08-11 case: "Barbados National Energy Company" → no match, but it already existed under its pre-2025 name as `Barbados National Oil Company` / BNOCL, id 100002005362). **Run it bare — do NOT pass `--country`.** Entities are shared across countries, so the developer of a project in one country routinely already exists on a project in another; `--country` only *annotates* matches (in-country vs. elsewhere) and never hides one, but running bare keeps the result unambiguous. (A real 2026 case: `entity_lookup.py "LNG Alliance" --country Singapore` was read as not-found and the entity was wrongly staged as new — it already existed on an India terminal. The script now refuses to hide such a match, but run bare regardless.)
...
3. If no match is found locally **or in `entity_history`**, add to `entity_additions` sheet with the lookup attempts logged (record that both the bare-local and `--pg` checks ran; a `--remote` run is a supplement and its `skipped_no_base_url` never substitutes for `--pg`)
