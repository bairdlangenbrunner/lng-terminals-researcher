...
Last revised: 2026-06-09 (rev 9)

Revision notes:
- rev 9 (2026-06-09): workbook tabs renamed for clarity. The reconciliation `routing` tab is now **`to_follow_up_on`** (the non-edit follow-up leads; the resolved, paste-ready edits stay in `edits_to_gem`). (Update-mode tabs also renamed: `updates`→`updates_summary`, `updates_all_fields`→`updates_in_database_format`.) Tab roles and contents are unchanged — only the names; references below use the new names.
- rev 8 (2026-06-08): the reconciliation output's three overlapping "group-1" tabs were renamed by role and the research pass became an **executed** step (new §3.8.5). Renames: `giignl_diff_operating`→`audit_operating`, `giignl_diff_nonoperating`→`audit_nonoperating`, `giignl_to_action`→`routing`. The old `candidate_edits` (which just highlighted flagged rows) is replaced by `edits_to_gem` — a GEM-shaped PASTE VIEW carrying ONLY the rows the research pass resolved to a DB change, with resolved values in the real GEM columns (color-coded by source confidence), verified `[ref]` URLs in the paired ref cells, and a leftmost `_change` column stating what changed and why (with sources). `routing` now carries only the non-edit findings (Discovery / gem_only / ambiguous / narrative) — matched-with-disagreement value conflicts moved to `edits_to_gem`. `country_notes_contributions` is no longer produced in reconciliation mode (still emitted by update/discovery); GIIGNL errata now log to `qa_review` (§3.11). The research pass writes `resolution`/`edits`/`change_summary` into `staged_recon_verdicts.json`; project-level field edits use a blank `unit_name` so they apply to every unit-row.
...
Most prose findings (proposed/construction/expansion — *forward-looking* activity the operating-only table omits) do **not** enter the `audit_*` sheets; they land in `to_follow_up_on` with their target route (Discovery/Update), and §5.7 keeps a forward phase from being mistaken for a table disagreement.

**Structured owner-change and name-change (rename) data (optional per-finding arrays).** When a prose finding carries an ownership change (a stake acquisition/divestiture) or a rename, capture it as STRUCTURED data on the finding rather than burying it in `prose_finding` free-text. Two optional arrays on a `giignl_narrative_findings.json` finding (both documented in that file's `_schema_optional_arrays` comment; a finding without them behaves exactly as before):

- `owner_changes`: `[{ "entity": str, "action": "acquire"|"divest"|"add"|"remove", "pct": number|null, "stake_of": str, "counterparty": str, "anchor": str, "entity_lookup": "pending"|"done" }]` — `entity` is the party whose stake changes; `pct` is the stake percentage (null if unstated); `stake_of` is what the percentage is a stake of; `counterparty` is the other party; `anchor` is the year/`YYYY-MM`; `entity_lookup` records whether `entity_lookup.py` has been run to confirm the entity isn't a duplicate. `build_review_package.py` routes each entry **two places**: (a) a row in the **`entity_additions`** sheet so the (possibly new) entity goes through the dup-check path — `lookup_was_run` is set to `RUN entity_lookup` (yellow-flagged) when `entity_lookup=="pending"` so the researcher runs the check before creating the entity (no duplicate entities, per methodology); and (b) a `to_follow_up_on` row with `action_category="narrative_owner_delta"` summarizing the stake change with the finding's citation.
- `name_changes`: `[{ "old": str, "new": str, "gem_field": str, "anchor": str }]` — `gem_field` is the target GEM column (typically `OtherNames`). Each entry routes **two places**: (a) a row in the **`name_reconciliation`** sheet (`gem_terminal_name, old_name, new_name, gem_field, anchor, citation, action_category`; empty-sheet-omitted — only present when ≥1 `name_change` exists); and (b) a `to_follow_up_on` row with `action_category="narrative_name_delta"`.

Both arrays reuse the finding's `citation` and are **routing candidates only** — nothing is auto-applied (§3.8). The rename folds the former name into the GEM record's `OtherNames` through the normal Update workflow; the owner change adds the entity via the dup-checked `entity_additions` path. Worked example (2026 edition): Woodside Louisiana LNG (formerly Driftwood) — Stonepeak `acquire` 40% of the infrastructure company (counterparty Woodside, anchor 2025, `entity_lookup: pending`) and a `Driftwood → Woodside Louisiana LNG` rename into `OtherNames`.

**Operating-status corrections (the one prose finding that DOES touch the `audit_*` sheets).** Sometimes the narrative states that a train *listed in the operating table* isn't actually operating — Bontang p.31: "only Trains G and H currently in operation," while the table lists Train E (tagged `(Mothballed)`) AND Train F (untagged). Here the prose corrects GIIGNL's own operating-only table, so summing the table's untagged F into the GIIGNL operating capacity is wrong (it manufactured a bogus 8.7-vs-5.75 / 51% Bontang conflict; the truth is 5.8-vs-5.75). Record these in an agent-authored **`giignl_prose_corrections.json`** (next to `giignl_extracted.csv`; one `operating_status_corrections` entry per terminal, keyed country+site+section, listing the `nonoperating_units` with status + the `GIIGNL <year> narrative, p.<n>` citation). `report_diff.py` auto-discovers it (or `--prose-corrections`) and moves the named unit's report row out of the operating total into the non-op set BEFORE matching, so the unit corroborates GEM's non-op unit (Bontang F ↔ GEM idled F) instead of inflating the operating delta. Capacity NUMBERS are still taken from the table (§5.6); nothing is applied to GEM (§3.8) — this only makes the GIIGNL side internally consistent with GIIGNL's own prose. The same file also carries a `nonop_corroborations` section for the related case where a unit has *no* table row at all because it already ceased (NWS Train 2: GIIGNL drops it from the operating table, but the narrative names it as permanently ceased) — these clear the matching GEM non-op unit's "GEM has, GIIGNL doesn't" flag and fill `giignl_narrative_mention`, keyed by GEM terminal+unit name.

**Implementation status:** this pass is **agent-driven today** — the agent reads the narrative pages (the 2026 text layer is clean per Appendix A.1) and produces the structured candidates directly into TWO agent-authored files: (a) `giignl_narrative_findings.json` — the full catalog of forward-looking / status findings, each triaged against the fresh GEM export, web-verified against an authoritative non-GIIGNL source (url_verifier-passed), and classified `narrative_update` / `narrative_discovery` / `narrative_monitor` / `narrative_confirm_already_tracked`; `build_review_package.py` routes these into the `to_follow_up_on` sheet (with the verified `CITES:` citation, never auto-applied per §3.8). (b) `giignl_prose_corrections.json` — the subset that corrects the operating TABLE's status (consumed deterministically by `report_diff.py`). The genuine new terminals from the catalog are then staged as a normal Discovery batch (`new_terminals`/`monitor_list`/`entity_additions`), and the status changes as Update findings (pull the existing timeline first per the Update SOP before staging `status_timeline_additions`). Worked example (2026 edition): ~60 narrative findings → ~50 already tracked (confidence bump), 7 status changes (Mozambique shelved→construction, Darwin idled→operating, Nguya FLNG construction→operating, Qatar Ras Laffan T4/6 strike, Portovaya/Vysotsk sanctions idling, NLNG T7 progress), 2 genuine new terminals (Khor Al Zubair FSRU/Iraq, Buenaventura-Buga FSU/Colombia), 1 monitor (Tomakomai). `giignl_extract.py` parses the **tables only**; do not expect it to surface prose findings. A future helper (`giignl_prose_extract.py`, or an extension of `giignl_extract.py`) could pre-filter narrative paragraphs containing capacity/date/lifecycle keywords to focus the read — see the TODO item. Per Appendix A.2, the in-scope narrative pages for the 2026 edition are the liquefaction narrative (pp. 28–31) and the regasification narrative (pp. 48–52); always re-derive page ranges per edition.
...
**Pass 3: unit-level alignment within a matched project.** Project matching (passes 1–2) finds the right GEM project; this pass then tries to align each GIIGNL row to a *specific GEM unit*. The bridge is that a GEM unit name often appears as a token inside the GIIGNL site name — GIIGNL `Arzew GL1Z` ⊃ GEM unit `GL1Z`. A GEM unit is accepted for a report row when its normalized name is a token-subset of the report site name AND (the unit name is code-like, i.e. contains a digit, OR the capacities are within 25%). When ≥1 unit aligns, the match is `match_granularity = "unit"` and the diff carries a `unit_matches` list (per-unit GIIGNL-vs-GEM capacity comparison); the `audit_operating` sheet renders one row per aligned unit beneath the project-total row. When nothing aligns (GIIGNL's rows don't map to GEM's unit names — e.g. Taichung's `Taichung`/`Taichung Expansion` rows vs GEM's `Phase N` units), it stays `match_granularity = "project"` and only the project total is compared. Implemented as `_align_units` in `report_diff.py`. Arzew is the canonical 1:1 case (GIIGNL GL1Z/GL2Z/GL3Z ↔ GEM units, all agree; the retired `GL4Z` GEM has but GIIGNL doesn't lands in `audit_nonoperating`). **Project-spanning guard:** a unit alignment is accepted only when the GIIGNL row's capacity is at least as close to that unit's capacity as to the GEM project total — otherwise a single GIIGNL row that actually covers the whole terminal gets mis-pinned to one unit by a coincidental code token, producing a contradictory unit-level conflict beside an agreeing project total. Worked example: GIIGNL `Portovaya LNG T1 (+ Portovyy FSU)` = 1.5 MTPA (the whole terminal); without the guard it pinned to GEM unit `T1` (0.75) and emitted a spurious unit-level 100% disagreement next to the correct project-level 1.5-vs-1.5 match. With the guard, Portovaya stays `match_granularity = "project"`.

**Train-range pre-pass (one GEM unit spans several GIIGNL train rows).** GEM commonly models a SPAN of trains as ONE unit whose name encodes the range — `LNG Canada Terminal` → unit `Phase 1 (T1-T2)` (14.0), `Brunei LNG` → `T1-T5`, `Plaquemines` → `Phase 1 (T1–24)` — whereas GIIGNL splits the same span into per-train rows (`LNG Canada T1` 7.0 + `LNG Canada T2` 7.0). The plain token-subset alignment can't bridge this (GEM unit tokens `{phase,1,t1,t2}` aren't a subset of the report site `{canada}`), so `_align_units` first runs a range pre-pass (`_train_numbers`): each **operating** GEM unit naming **≥2 trains** absorbs the GIIGNL per-train rows whose train number falls inside its range and **sums their capacities** before comparing. So GIIGNL T1+T2 (7+7=14) aligns to GEM `Phase 1 (T1-T2)` (14) and **agrees** as one unit row — instead of T1 matching the project at half-capacity (a bogus 7-vs-14 50% conflict) and **T2 orphaning into `report_only` as a false "GIIGNL has, GEM doesn't"** (the LNG Canada bug this fixes). Report-row train numbers are read from the `trains` column first, then the site name. This pairs with the `giignl_extract.py` region-tolerant train-suffix strip (a trailing province like `, BC` no longer blocks `T1`/`T2` from moving into the `trains` column), which also lets the two GIIGNL rows roll up to one `LNG Canada` project and match GEM **exactly**. Single-train units and non-range unit codes (`GL1Z`, `Senboku II`) are untouched.

**Project-level fallback.** GIIGNL aggregates per-site and GEM splits into units, so when unit alignment fails the diff records the GIIGNL-side numbers against the GEM *project total* (operating-unit rollup). If that total doesn't match the GIIGNL number, it's a `value-disagreement`. Either way, only **operating** GEM capacity enters the comparison (GIIGNL's tables are operating-only) — non-operating units are split out to `audit_nonoperating` (§3.10).
...
These get routed to the `to_follow_up_on` sheet with `route: review` (not directly to Update) — the conclusion may be "GEM is correct, GIIGNL gap" rather than an actionable edit.
...
- `audit_operating` / `audit_nonoperating` sheets — every diff finding with full context (both sides' values, classification, category) — the evidence layer
- `to_follow_up_on` sheet — non-edit findings that route to Discovery / Review (GIIGNL-only, gem_only, ambiguous, narrative), with target action noted
- `edits_to_gem` sheet — the rows the §3.8.5 research pass resolved to a DB change, with researched values already filled (see §3.8.5)

"No auto-apply" does not mean "no resolution." It means GIIGNL's number is never the *source* for the new value — an independent primary/regulatory source is. The §3.8.5 research pass below does that resolution inside the reconciliation batch; per `docs/reference/source_roster.md`, GIIGNL is Tier 1 but not authoritative, so sponsor IR or primary regulatory filings take priority and are what gets cited.

### §3.8.5 Execute the research pass (resolve disagreements in-batch)

Reconciliation used to stop at "route to Update" — flag the disagreement and defer the actual research. That left the paste view proposing nothing. The research pass is now an **executed step**: for every matched-with-disagreement finding that is a *material* conflict, the agent researches and reaches a conclusion here, and the build turns it into a real proposed edit.

**Scope (what to research vs. let default to no_change).** Material = a `≥5%` capacity delta, or a non-benign owner change (a genuine corporate transaction — acquisition, divestment, exit — not a JV-vs-immediate-owner naming variant, a parent-vs-subsidiary relabel, or a metric-basis capacity difference like nameplate-vs-sendout or peak-vs-average). The benign majority resolve to `no_change` with a one-line basis; only the genuine changes get researched into edits. (The `matches_with_disagreement` count is inflated by the no-tolerance-band rule — most are benign; see §6.)

**Procedure per material finding:**
1. Note GEM's own cited source/edition (`gem_capacity_source` in the diff JSON) and what GIIGNL asserts.
2. `WebSearch` / `WebFetch` the operator, sponsor IR, or national regulator (FERC / DOE / EIA / Norwegian Offshore Directorate / etc.) for the explicit value.
3. **Verify every candidate URL with `python scripts/url_verifier.py "<url>" "<claimed-value>"`** — pass the actual claimed value (owner name, capacity number) as the token. A page that loads but lacks the value is a failed citation, not a source. Require **≥2 independent corroborations, or one primary/regulatory source** (→ green). A lone non-primary source is yellow; if it can't be corroborated, resolve to `no_change` and log a `staged_qa_review.json` entry — never write GIIGNL's figure as the new value. **Never** cite gem.wiki / globalenergymonitor.org or a GEM-derived republisher (Wikipedia / IEEFA / news footnoting GEM).
4. Run `python scripts/entity_lookup.py "<entity>"` before staging any new owner/operator/parent name (entities are shared across trackers — don't create duplicates). A major company missing from the local export but established elsewhere should be linked to its existing entity record at apply time; note that in `change_summary` rather than treating it as a new-entity creation.
5. Write the conclusion into `batches/staging/recon/giignl<YEAR>/staged_recon_verdicts.json`, keyed by `(terminal_id, section_type, unit_name)`:

```jsonc
{
  "terminal_id": "T1000...", "section_type": "liquefaction", "unit_name": "",
  "facet": "owner", "confidence": "high",
  "resolution": "edit",                       // "edit" | "no_change"
  "edits": [                                   // one per field actually changing
    { "gem_field": "Owner",
      "new_value": "VTTI [50%]; Shell PLC [50%]",
      "ref_urls": ["https://...primary...", "https://...corroboration..."] }
  ],
  "change_summary": "Owner: replace Ancala with VTTI (VTTI completed acquisition of Ancala's 50% 2024-08-09); Shell unchanged.",
  "basis": "deeper evidence prose shown in audit_operating",
  "verdict": "RESOLVED (owner edit → see edits_to_gem). ...",
  "sources": ["https://...", "https://..."]
}
```

`change_summary` is the short leftmost-column text in `edits_to_gem`; `verdict`/`basis` are the longer evidence shown in `audit_operating`. **Project-level fields (Owner / Operator / Parent / Capacity at project granularity) use a blank `unit_name`** so the build writes the resolved value to every unit-row of the project (the export duplicates project-level fields across unit-rows — edits must too). A `unit_name`-scoped verdict applies only to that unit-row.
...
- `audit_operating` — OPERATING match audit (the evidence layer): GEM operating capacity vs GIIGNL's operating-only tables. One project-total row per match (exact + fuzzy); for `match_granularity = "unit"` matches (§3.5), one per-unit row is emitted beneath it (the `level` column distinguishes `project` from `unit` rows). `gem_unit_name` lists **operating units only** (so the unit list reconciles with the operating capacity). `report_sites_merged` lists folded rows (expansion or unit-code, §3.5). **Red fill** marks conflicting cells: the `disagreements` cell, the four capacity columns when capacity differs at all (any non-zero delta), `owners_report_only`/`owners_gem_only` when non-empty, and a per-unit row's capacity cells when that unit disagrees. **Capacity conflicts are graded by size** — light red (FFE5E5) for a `<5%` delta, darker red (FFB0B0) for `>=5%` (or undefined when GEM capacity is 0); owner-only deltas stay light red (§4). Fuzzy matches get a yellow `confidence` cell. (Triggers mirror `report_diff.py`'s disagreement logic; keep in sync.) Findings whose §3.8.5 verdict resolves to a DB change become resolved rows in `edits_to_gem`; no-change conclusions live here only.
- `audit_nonoperating` — non-operating units (proposed/construction/shelved/cancelled/idled/mothballed/retired) of **matched** projects. GIIGNL's tables are operating-only, so each row defaults to a light-red `gem_only_flag` = "GEM has, GIIGNL doesn't" UNLESS the §3.2.1 narrative pass populated `giignl_narrative_mention` (a confirmed forward phase → no conflict per §5.7; left unfilled). Worked example: Taichung Phase 3 (in the p.52 narrative → no highlight) vs Phase 4 (absent everywhere → highlighted); Arzew `GL4Z` retired → highlighted (GIIGNL drops retired).
- `to_follow_up_on` — NON-edit findings with proposed routing: GIIGNL-only → Discovery, gem_only → review, ambiguous → disambiguate, plus narrative findings. Matched-with-disagreement value conflicts are NOT here — those go to `edits_to_gem` once researched.
- `edits_to_gem` — the PASTE VIEW (the actionable deliverable). GEM-CSV-shaped; ONLY the unit-rows the §3.8.5 research pass resolved to a DB change. For each, the resolved value is written into the real GEM column (colored green/yellow/red by source confidence), the verified corroborating URL(s) into the paired `<field> [ref]` column, GIIGNL's number kept as a cell comment (reference only — never pasted), and a leftmost `_change` column states what changed and why (with sources). No-change rows are omitted. If nothing resolved to an edit, the sheet says so.

Empty sheets are omitted per `docs/reference/workbook_conventions.md`. `country_notes_contributions` is intentionally NOT produced in reconciliation mode (update/discovery only).
...
When reconciliation finds GIIGNL clearly wrong (e.g. capacity figure that contradicts the operator's own filings), log it in `qa_review` (a `staged_qa_review.json` entry, type "GIIGNL errata observed") — `country_notes_contributions` is no longer produced in reconciliation mode. Over multiple years this builds a useful record of GIIGNL's failure modes (which countries it covers poorly, which kinds of fields it gets wrong) that informs future reconciliations.
...
This sheet contains only matched projects (operating side); GEM-only and GIIGNL-only *projects* live in `to_follow_up_on`, and matched-with-disagreement value conflicts resolved to an edit live in `edits_to_gem` (§3.8.5).
