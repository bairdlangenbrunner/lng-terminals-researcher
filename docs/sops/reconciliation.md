...
Most prose findings (proposed/construction/expansion — *forward-looking* activity the operating-only table omits) do **not** enter `giignl_diff`; they land in `giignl_to_action` with their target route (Discovery/Update), and §5.7 keeps a forward phase from being mistaken for a table disagreement.
