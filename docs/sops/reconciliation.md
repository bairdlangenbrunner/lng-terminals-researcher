...
Most prose findings (proposed/construction/expansion — *forward-looking* activity the operating-only table omits) do **not** enter the `audit_*` sheets; they land in `routing` with their target route (Discovery/Update), and §5.7 keeps a forward phase from being mistaken for a table disagreement.
...
These get routed to the `routing` sheet with `route: review` (not directly to Update) — the conclusion may be "GEM is correct, GIIGNL gap" rather than an actionable edit.
...
- `routing` sheet — non-edit findings that route to Discovery / Review (GIIGNL-only, gem_only, ambiguous, narrative), with target action noted
...
The Update / Discovery batches still consume the `routing` findings that aren't DB edits (GIIGNL-only → Discovery, gem_only → review), citing GIIGNL as ONE source among others.
...
- `routing` — NON-edit findings with proposed routing: GIIGNL-only → Discovery, gem_only → review, ambiguous → disambiguate, plus narrative findings. Matched-with-disagreement value conflicts are NOT here — those go to `edits_to_gem` once researched.
...
This sheet contains only matched projects (operating side); GEM-only and GIIGNL-only *projects* live in `routing`, and matched-with-disagreement value conflicts resolved to an edit live in `edits_to_gem` (§3.8.5).
