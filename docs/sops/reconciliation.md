...
**FSRU terminals — operating-vessel capacity only.** GIIGNL lists every recently-deployed FSRU at a berth as a separate operating row, but GEM models a berth that cycled through vessels as *sequential* — one operating unit (the current vessel) plus the superseded vessels as `retired`/`idled` units. Naively summing GIIGNL's rows compares GIIGNL's lifetime-of-vessels against GEM's currently-operating vessel. For an FSRU project, `_fsru_operating_report_capacity` instead aligns each GIIGNL FSRU row to a GEM unit **by vessel name** (GEM's `unit_name` is the vessel identity) and sums only the rows mapping to a GEM *operating* unit; rows mapping to a retired/idled GEM unit, or to no GEM unit, become per-vessel notes in `disagreements` (a status/discovery signal, not a capacity delta). Worked example — Ain-Sokhna: GIIGNL lists Energos Power + Höegh Galleon + Energos Eskimo (3×5.7=17.1), but GEM has only Energos Power operating (5.51, with BW Singapore + Höegh Gallant retired); the diff compares **5.7 vs 5.51** and notes the two non-operating GIIGNL vessels. (If GEM in fact runs several FSRUs concurrently, all map to operating units and the full sum is compared — the rule self-adjusts.)
