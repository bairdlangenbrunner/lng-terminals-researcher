...
ADJUDICATED_OVERTURNS = {
    # Tango FLNG: recovery pass (recovery_golar-eni.json) confirmed INSUFFICIENT —
    # no Tango-specific power-system source exists in reach (EXMAR fleet page dead,
    # Nguya's genset disclosure explicitly does not extend to Tango). Existing
    # live-DB True KEPT: off-grid moored FLNG physics prior + no contrary evidence;
    # the contentless marinelink ref is flagged in qa_review (no staged row = no
    # ref edit lane to repair it in this batch).
    "T100000131003": "keep",
}

# Verdict/record patches ruled at the gate; "gate_note" is prepended to notes.
VERDICT_OVERRIDES = {
    "T100000131126": {  # Richards Bay Transnet FSRU (ZET) — active proposed FSRU
        "verdict": "INSUFFICIENT",
        "confidence": "",
        "gate_note": ("QC-GATE 2026-08-02: downgraded NO -> INSUFFICIENT under the "
                      "FSRU-onboard convention (Hadera class): the documented "
                      "supply-out relationship to Eskom's Richards Bay CCGT does not "
                      "answer the (future) vessel's own onboard-generation question; "
                      "no vessel assigned yet. The cancelled Karpowership FSRUs "
                      "(Coega/Richards Bay/Saldanha) and Mossel Bay keep their "
                      "design-based NOs — no vessel was ever assigned and the "
                      "licences/projects are dead, so the vessel-machinery question "
                      "is moot there."),
    },
}
...
REF_DROPS = {
    "T100000131123": {"drop_existing": {  # Nguya FLNG
        "https://www.youtube.com/shorts/lGBy3NoLdUQ":
            "YouTube Shorts clip; no captive-power content supporting the claim",
    }},
    "T100000131003": {"drop_existing": {  # Tango FLNG (applies only if a row stages)
        "https://www.marinelink.com/news/tango-flng-excalibur-fsu-set-begin-508936":
            "verifier-proven contentless for the claim (page lacks 'generator'/'power generation')",
    }},
}
