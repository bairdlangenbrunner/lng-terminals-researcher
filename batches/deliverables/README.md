...
| `location_accuracy_audit_20260729_1146_ET.xlsx` | Coordinate-accuracy worklist for the location-improvement goal in the current update pass: all 492 terminals carrying `Accuracy = approximate`, split by edit recency — 347 with a 2026 `LastUpdated` (`recent_approximate`) vs 145 last touched 2025 or earlier (`stale_approximate`), plus 3 with `Accuracy` blank. Each row links to the live project-edit page. Rebuildable any time via `python scripts/location_accuracy_audit.py` after a fresh pull (counts drift as staff edit the DB). |
