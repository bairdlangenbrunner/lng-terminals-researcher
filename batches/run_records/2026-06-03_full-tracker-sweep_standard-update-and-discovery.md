...
## Re-verify follow-up (2026-06-04 evening) — sourcing fix

User mandate after reviewing the sweep: (1) NEVER cite gem.wiki/globalenergymonitor.org (circular — GEM's own publication); (2) every staged value needs ≥2 independent working URLs that each explicitly contain the value (3 when findable). Both are now CLAUDE.md hard requirements + in the update/discovery briefs (permanent). Also found + fixed a README bug: the "Countries checked" list omitted any country whose only output was a country-less discovery `qa` note (e.g. **United States**) — `build_review_package.py` gained `--checked-roster` (fed from the per-country done-markers by `_build_region.py`) so every swept country is listed.

Ran a targeted re-verification (not a fresh re-sweep of qa-only countries): 125 slug-groups / 238 reference-bearing files, one agent each (`_resweep_reverify_brief.md` + `_resweep_reverify_wf.js`), re-sourcing every staged value off-GEM. **Result:** 0 gem.wiki/globalenergymonitor URLs in any citation field tree-wide; values re-sourced to ≥2 independent verifier-PASS URLs where corroborable, single-source kept yellow, gem.wiki-only/GEM-inference values blanked + qa'd (cambodia ×8, canada_1_east ×13 lifecycle-year refs, PNG T3 FID, others). All 12 workbooks rebuilt with fresh stamps (europe/africa/americas `_2108/2109`, asia `_2114`, middleeast `_2124`, oceania `_2129`), recalc-clean. Hit the session limit twice (12:40pm, 1am resets) — built a CronCreate auto-resume mechanism (`_reverify_state.py --reset`), though both limits cleared early and the pass finished live with zero lost work.
