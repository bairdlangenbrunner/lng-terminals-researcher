...
4. `entity_lookup.py --remote` is BROKEN: `DEFAULT_BASE_URL` is the stale
   `gem-project-db.herokuapp.com` host (same dead host as `fetch_timeline.py`), so remote
...
- 2026-07-02 ~13:55 ET — DISCOVERY WAVE COMPLETE: 39/39 disc done markers verified (25 covered +
  ...
  entity_lookup.py --remote confirmed BROKEN (stale gem-project-db.herokuapp.com host, same as
