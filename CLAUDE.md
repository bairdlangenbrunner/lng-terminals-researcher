...
| **Captive-power cross-tracker** (on-request; LNG terminal ↔ GOGPT captive gas power plant) | "captive power", "which terminals have a captive power plant", "match LNG to GOGPT", "colocated gas power" | `docs/workflows.md` §9 + Captive-power SOP; `scripts/captive_power_colocation.py` match→research→stage; **LNG-side edits only** (`CaptiveGasPower`/`PowerPlantsSupplied` + status/FID) — GOGPT gaps go in a memo, never staged |
...
- Captive-power matching stages LNG-side only. A "captive gas power plant" is on-site, >50 MW, powers the terminal/liquefaction — **partially-captive AND mechanical-drive both count** (mechanical-drive = on-site gas turbines that drive the refrigeration compressors without generating electricity; user's call 2026-07-09, makes grid-fed sites like Woodside Louisiana in-scope). GOGPT is usually ahead of the LNG tracker on flagging, and its aggregate "power station" MW may conflate mechanical-drive compressors with a dedicated electric plant — flag that in the memo, never bend the LNG side to match it.
...
Timeline reads: `fetch_timeline.py` now reads the full ordered timeline straight from the **read-only Postgres** (`GEM_READONLY_DB_URL`, default path) — so a status change is **never** blocked for want of a timeline. The old web scraper (legacy `--web` flag) hit a Heroku host that is now 404; it's off by default and only needed if the read-only DB itself is unreachable. **Do NOT punt a confirmed status change to a qa note "because the timeline tool is down" — pull the timeline from Postgres and stage the edit.** (This is the Quynh Lap miss: construction was found but the status edit was wrongly deferred to a qa note.) The qa-note fallback applies only in the rare case where the read-only DB AND any live web host are both unreachable.
